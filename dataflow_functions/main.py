...
def get_paths(directory):
    ...
    sub_directories = tf.io.gfile.listdir(directory)

    if get_config()['test_mode']:
        sub_directories = sub_directories[get_config()['test_mode_sample']]
...
def get_embeddings(text):
    ...
    embed = hub.Module(get_config()['module_url'])
...
def preprocessing_fn(input_features):
    ...
    # tokenize text
    text_tokens = tf.string_split( text, get_config()['delimiters'] )

    # bag of words (bow) indices
    text_tokens_indices = tft.string_to_int(text_tokens, top_k=get_config()['vocab_size'] )

    # tf.idf
    bag_of_words_indices, tf_idf = tft.tfidf(text_tokens_indices, get_config()['vocab_size'] + 1)
...
#####################################################################################################
def run_transformation_pipeline(pipeline_options):

    # with beam.Pipeline(runner, options=pipeline_options) as pipeline:
    with beam.Pipeline(options=pipeline_options) as pipeline:

        with impl.Context( get_config()['temp_dir'] ):
            ...
            ### read raw train data
            raw_train_data = read_raw_data(pipeline, get_config()['data_dir'], step)
            ...
            write_to_files(
                ...
                , get_config()['TRANSFORMED_TRAIN_DATA_FILE_PREFIX']
                , get_config()['TRANSFORMED_ENCODING']                             
                , step
            )
            ...
            ### read raw eval data
            raw_eval_data = read_raw_data(pipeline, get_config()['RAW_EVAL_DATA_DIR'], step)
            ...
            write_to_files(
                transformed_train_dataset
                , get_config()['TRANSFORMED_EVAL_DATA_FILE_PREFIX']
                , get_config()['TRANSFORMED_ENCODING']
                , step
            )
        ...
        ################## write transformation artefacts ##################
            ...
            (
                ...
                get_config()['TRANSFORM_ARTEFACTS_DIR']
                )
            )